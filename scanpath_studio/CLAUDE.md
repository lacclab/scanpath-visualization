...
- [tour.py](tour.py) — first-visit welcome tour: a 6-step `st.dialog` opened once per session from `main()` (suppressed for `?embed=true` and deep links via `tour_suppressed`), replayable from the sidebar's **🎓 Show tutorial** button. `tour_seen` is set *before* opening — see the module docstring before changing this — and Back/Next navigate via `on_click` callbacks (fragment reruns), Skip/Done close via full-app `st.rerun()`.
