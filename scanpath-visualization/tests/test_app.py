...
class TestClampCanvasSize:
    ...
    def test_clamp_canvas_size_normal(self, normalized_words_df, normalized_fixations_df):
...
class TestComputeTrialStats:
    ...
    def test_compute_trial_stats_basic(self, normalized_words_df, normalized_fixations_df):
...
class TestGatherTrialMetadata:
    ...
    def test_gather_trial_metadata_single_value(self, normalized_words_df, normalized_fixations_df):
    ...
    def test_gather_trial_metadata_numeric(self, normalized_words_df, normalized_fixations_df):
    ...
    def test_gather_trial_metadata_missing_field(self, normalized_words_df, normalized_fixations_df):
        metadata = gather_trial_metadata(
            normalized_words_df, normalized_fixations_df, ["nonexistent_field"]
        )
        assert len(metadata) == 0

    def test_gather_trial_metadata_multiple_fields(self, normalized_words_df, normalized_fixations_df):
        normalized_words_df["difficulty_level"] = ["Adv", "Adv", "Adv"]
        normalized_fixations_df["pass_index"] = [1, 1, 1]
        metadata = gather_trial_metadata(
            normalized_words_df, normalized_fixations_df, ["difficulty_level", "pass_index"]
        )
...
class TestBuildComparisonOptions:
    ...
    def test_build_comparison_options_participant_mode(self):
        ...
        options = _build_comparison_options(
            combos, "Participant", "p1", "t1", "para1"
        )
        assert len(options) > 0
